...
records its on_progress callbacks so the SSE endpoint can stream them. Jobs are
TTL-evicted and capped so a public URL cannot grow memory forever.
...
def _run(job: Job, q: pb.StructuredQuery) -> None:
    ...
    try:
        job.result = pb.build(q, on_progress=on_progress)
